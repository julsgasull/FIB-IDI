...
void MyGLWidget::carregaShaders()
{
  ...
  // Obtenim identificador per als atributs del vertex shader
  ...
  tipusFocusLoc   = glGetUniformLocation (program->programId(), "focusEscena");   // bool
}
...
{
  posF = glm::vec3(0.0, -0.5, 0.0);  // sobre del patricio, a 0.5 sobre el terra (-1)
  focusEsc = true;
  llumEncesa = true;
  posicioFocus();
  colorFocus();
...
{
  glm::vec3 colF;
  if (llumEncesa) colF = glm::vec3(1, 1, 1);
  else colF = glm::vec3(0, 0, 0);
...
{
  //glViewport (0, 0, width()*2, height()*2); // Aquesta crida no caldria perquè Qt la fa de forma automàtica amb aquests paràmetres
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  escala = 0.3 /(maxy-miny); // patricio fa 0.3 alçada
  centrePatr[0] = (minx+maxx)/2.0;    centrePatr[1] = (miny+maxy)/2.0;  centrePatr[2] = (minz+maxz)/2.0;
  centreBasePatr[0] = centrePatr[0];  centreBasePatr[1] = miny;         centreBasePatr[2] = centrePatr[2];
}
...
void MyGLWidget::keyPressEvent (QKeyEvent* event)
{
  ...
  {
    case Qt::Key_O:
      perspectiva = !perspectiva; projectTransform ();     // canvia òptica entre perspectiva i axonomètrica
      break;
    /*
    case Qt::Key_K:
      posF.x -= 0.1; posicioFocus();    // focus esquerra
      break;
    case Qt::Key_L:
      posF.x += 0.1; posicioFocus();    // focus dreta
      break;
    case Qt::Key_I:
      posF.y += 0.1; posicioFocus();    // focus amunt
      break;
    case Qt::Key_M:
      posF.y -= 0.1; posicioFocus();    // focus abaix
      break;
    */
    case Qt::Key_F:
      focusEsc = !focusEsc; defineFocus();
      break;
    case Qt::Key_Left:
      movePat[0] -= 0.1; paintGL();     // patricio esquerra
      posF[0] -= 0.1; posicioFocus();   // focus esquerra
      break;
    case Qt::Key_Right:
      movePat[0] += 0.1; paintGL();     // patricio dreta
      posF[0] += 0.1; posicioFocus();   // focus dreta
      break;
    case Qt::Key_Up:
      movePat[2] -= 0.1; paintGL();     // patricio amunt
      posF[2] -= 0.1; posicioFocus();   // focus amunt
      break;
    case Qt::Key_Down:
      movePat[2] += 0.1; paintGL();     // patricio abaix
      posF[2] += 0.1; posicioFocus();   // focus abaix
      break;
    case Qt::Key_B:
      backFaceCulling = !backFaceCulling;   // canviem valor
      if (backFaceCulling)  glEnable  (GL_CULL_FACE);
      else                  glDisable (GL_CULL_FACE);
      break;
    case Qt::Key_L:
      llumEncesa = !llumEncesa; colorFocus();
      break;
