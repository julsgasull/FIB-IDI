...
void MyGLWidget::paintGL ()
{
  ...
  glBindVertexArray (VAO_Terra);
  modelTransformTerra (); glDrawArrays(GL_TRIANGLES, 0, 12);

  // Activem el VAO per a pintar el Patricio
  glBindVertexArray (VAO_Patr);
  modelTransformPatricio1 (); glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);
  modelTransformPatricio2 (); glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);
